...
public class FHIRModelInfoBuilder extends ModelInfoBuilder {
    ...
    private List<ClassInfo> getFhirElementInfos() {
        return this.typeInfos.stream()
                .map(x -> (ClassInfo) x)
                .filter(x ->
                        x != null && x.getBaseType() != null && x.getBaseType().equals("FHIR.Element"))
    ...
    protected void beforeBuild() {
        ...
        // ClassInfo sd = fhirElementInfos.stream().map(x -> (ClassInfo)x).filter(x ->
        // x.getName().equals("dateTime")).findFirst().get();
        fhirElementInfos.stream()
                .filter(x -> x.getElement().get(0).getElementType() != null)
                .map(x -> new ConversionInfo()
                        .withFromType("FHIR." + x.getName())
                        .withToType(x.getElement().get(0).getElementType())
                        .withFunctionName("FHIRHelpers.To"
                                + this.unQualify(x.getElement().get(0).getElementType())))
