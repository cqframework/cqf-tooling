...
public class VSACValueSetGenerator extends Operation {
    ...
    public void execute(String[] args) {
        ...
        for (String arg : args) {
            ...
            switch (flag.replace("-", "").toLowerCase()) {
                ...
                case "metasheetnum":
                case "msn":
                    metaSheetNum = Integer.valueOf(value);
                    break;
                case "metanamerow":
                case "mnr":
                    metaNameRow = Integer.valueOf(value);
                    break;
                case "metaoidrow":
                case "mor":
                    metaOidRow = Integer.valueOf(value);
                    break;
                case "metaversionrow":
                case "mvr":
                    metaVersionRow = Integer.valueOf(value);
                    break;
                case "metastewardrow":
                case "msd":
                    metaStewardRow = Integer.valueOf(value);
                    break;
                case "metaclinicalfocusrow":
                case "mcfr":
                    metaClinicalFocusRow = Integer.valueOf(value);
                    break;
                case "metascoperow":
                case "msr":
                    metaDataElementScopeRow = Integer.valueOf(value);
                    break;
                case "metainclusionrow":
                case "mir":
                    metaInclusionRow = Integer.valueOf(value);
                    break;
                case "metaexclusionrow":
                case "mer":
                    metaExclusionRow = Integer.valueOf(value);
                    break;
                case "codesheetnum":
                case "csn":
                ...
                case "includecompose":
                case "ic":
                    includeCompose = value.toLowerCase().equals("true") ? true : false;
                    break;
                case "includeexpansion":
                case "ie":
                    includeExpansion = value.toLowerCase().equals("true") ? true : false;
                    break;
                case "declarecpg":
                case "cpg":
        ...
        try {
            ...
            if (includeExpansion) {
                ...
                if (declareCPGProfiles) {
                    ...
                    vs.addExtension()
                            .setUrl("http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-knowledgeCapability")
                            .setValue(new CodeType("executable"));
                    vs.addExtension()
                            .setUrl("http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-knowledgeRepresentationLevel")
                            .setValue(new CodeType("executable"));
                    vs.addExtension()
                            .setUrl("http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-usageWarning")
                            .setValue(
                                    new StringType(
                                            "This value set contains a point-in-time expansion enumerating the codes that meet the value set intent. As new versions of the code systems used by the value set are released, the contents of this expansion will need to be updated to incorporate newly defined codes that meet the value set intent. Before, and periodically during production use, the value set expansion contents SHOULD be updated. The value set expansion specifies the timestamp when the expansion was produced, SHOULD contain the parameters used for the expansion, and SHALL contain the codes that are obtained by evaluating the value set definition. If this is ONLY an executable value set, a distributable definition of the value set must be obtained to compute the updated expansion."));
                }
            }
            // writeValueSetToFile(vs.getTitle() != null ? vs.getTitle().replaceAll("\\s", "").concat("." + encoding) :
            // "valueset".concat("." + encoding), vs);
            writeValueSetToFile("valueset-" + vs.getId() + "." + encoding, vs);
        } catch (IllegalArgumentException e) {
            throw new IllegalArgumentException(String.format(
                    "%s - ValueSet: %s",
                    e.getMessage(), (vs.getTitle() == null || vs.getTitle().equals("") ? "undefined" : vs.getTitle())));
        }
    }
    ...
    public void resolveMetaData(ValueSet vs, Workbook workbook) {
        ...
        if (id == null || id.equals("")) {
            throw new IllegalArgumentException(String.format(
                    "No id value found for ValueSet: %s",
                    vs.getTitle() == null || vs.getTitle().equals("") ? "untitled" : vs.getTitle()));
        }
        ...
        if (declareCPGProfiles) {
            ...
            vs.addExtension()
                    .setUrl("http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-knowledgeCapability")
                    .setValue(new CodeType("shareable"));
            vs.setExperimental(false);
            vs.addExtension()
                    .setUrl("http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-knowledgeRepresentationLevel")
                    .setValue(new CodeType("narrative"));
            vs.getMeta().addProfile("http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-publishablevalueset");
            vs.addExtension()
                    .setUrl("http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-knowledgeCapability")
                    .setValue(new CodeType("publishable"));
        }

        String inclusion = getSecondStringInRow(metaSheet, metaInclusionRow);
        if (inclusion != null) {
            vs.addExtension(new Extension()
                    .setUrl("http://fhir.org/guides/cdc/opioid-cds/StructureDefinition/cdc-valueset-inclusion")
                    .setValue(new StringType(inclusion)));
        }

        String exclusion = getSecondStringInRow(metaSheet, metaExclusionRow);
        if (exclusion != null) {
            vs.addExtension(new Extension()
                    .setUrl("http://fhir.org/guides/cdc/opioid-cds/StructureDefinition/cdc-valueset-exclusion")
                    .setValue(new StringType(exclusion)));
        }
    }
        ...
        while (it.hasNext()) {
            ...
            if (code.matches("[+-]?\\d(\\.\\d+)?[Ee][+-]?\\d+")) {
                throw new IllegalArgumentException(
                        String.format("Scientific Notation is not allowed for a code: %s", code));
            }

            if ((version == null || version.equals(""))
                    && (code == null || code.equals(""))
                    && ((systemName == null || systemName.equals(""))
                            && (SpreadsheetHelper.getCellAsString(row.getCell(systemOidCol)) == null
                                    || SpreadsheetHelper.getCellAsString(row.getCell(systemOidCol))
                                            .equals("")))) {
                // Protecting against error where last line has no content except hidden characters introduced by
            ...
            if (systemName == null || systemName.equals("")) {
                ...
                if (system == null || system.equals("")) {
                    throw new IllegalArgumentException(
                            String.format("No system value found on row: %d", row.getRowNum()));
                }
