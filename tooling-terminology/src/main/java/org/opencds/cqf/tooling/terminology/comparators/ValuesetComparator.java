...
public class ValuesetComparator extends Comparator {
    public void compareValueSets(
            ValueSet terminologyServerValueSet, ValueSet sourceOfTruthValueSet, Map<String, Object> vsFailureReport) {
        ...
        if (!terminologyServerValueSet.getUrl().equals(sourceOfTruthValueSet.getUrl())) {
            ...
            urlFailure.put(
                    "URL",
                    terminologyServerValueSet.getUrl() + "|" + terminologyServerValueSet.getVersion()
                            + " Does not equal IG URL " + sourceOfTruthValueSet.getUrl() + newLine);
        ...
        if (!terminologyServerValueSet.getVersion().equals(sourceOfTruthValueSet.getVersion())) {
            ...
            versionFailure.put(
                    "Version",
                    "\"" + terminologyServerValueSet.getVersion() + "\" Does not equal IG Version \""
                            + sourceOfTruthValueSet.getVersion() + "\"" + newLine);
        ...
        if (!terminologyServerValueSet.getStatus().equals(sourceOfTruthValueSet.getStatus())) {
            ...
            statusFailure.put(
                    "Status",
                    "\"" + terminologyServerValueSet.getStatus() + "\" Does not equal IG Status \""
                            + sourceOfTruthValueSet.getStatus() + "\"" + newLine);
        ...
        if (!terminologyServerValueSet.getExperimental() == sourceOfTruthValueSet.getExperimental()) {
            ...
            experimentalFailure.put(
                    "Experimental",
                    "\"" + terminologyServerValueSet.getExperimental() + "\" Does not equal IG Experimental \""
                            + sourceOfTruthValueSet.getExperimental() + "\"" + newLine);
        ...
        if (!terminologyServerValueSet.getName().equals(sourceOfTruthValueSet.getName())) {
            ...
            nameFailure.put(
                    "Name",
                    "\"" + terminologyServerValueSet.getName() + "\" Does not equal IG Experimental \""
                            + sourceOfTruthValueSet.getName() + "\"" + newLine);
        ...
        if (!terminologyServerValueSet.getTitle().equals(sourceOfTruthValueSet.getTitle())) {
            ...
            titleFailure.put(
                    "Status",
                    "\"" + terminologyServerValueSet.getTitle() + "\" Does not equal IG Experimental \""
                            + sourceOfTruthValueSet.getTitle() + "\"" + newLine);
        ...
        if (!terminologyServerValueSet.getPublisher().equals(sourceOfTruthValueSet.getPublisher())) {
            ...
            publisherFailure.put(
                    "Publisher",
                    "\"" + terminologyServerValueSet.getPublisher() + "\" Does not equal IG Experimental \""
                            + sourceOfTruthValueSet.getPublisher() + "\"" + newLine);
        ...
        if (!fieldsWithErrors.isEmpty()) {
            vsFailureReport.put(
                    terminologyServerValueSet.getUrl() + "|" + terminologyServerValueSet.getVersion() + " - "
                            + terminologyServerValueSet.getName(),
                    fieldsWithErrors);
        }
    }

    private boolean compareComposes(
            ValueSet.ValueSetComposeComponent terminologyServerComposeComponent,
            ValueSet.ValueSetComposeComponent sourceOfTruthComposeComponent) {
                ...
                terminologyServerIncludesMap.forEach((terminologyIncludeKey, terminologyIncludeValue) -> {
                    if (sourceOfTruthIncludesMap.containsKey(terminologyIncludeKey)) {
                        terminologyServerIncludesMap.forEach((terminologyIncludesKey, terminologyIncludesValue) -> {
                            Map<?, ?> terminologyConceptsMap =
                                    (HashMap) (terminologyServerIncludesMap.get(terminologyIncludeKey));
                            Map<?, ?> truthConceptsMap =
                                    (HashMap) (sourceOfTruthIncludesMap.get(terminologyIncludeKey));
                            if (!terminologyConceptsMap.isEmpty()
                                    && !truthConceptsMap.isEmpty()
                                ...
                                terminologyConceptsMap.forEach((terminologyConceptsKey, terminologyConceptsValue) -> {
                                    if (truthConceptsMap.containsKey(terminologyConceptsKey)) {
                                        String truthConceptsValue =
                                                (String) (truthConceptsMap.get(terminologyConceptsKey));
