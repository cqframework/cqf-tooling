...
public class ConfigValueSetGenerator implements ExecutableOperation {
    ...
    @OperationParam(
            alias = {"pathtoconfig", "ptc"},
            setter = "setPathToConfig",
            required = true,
    ...
    @OperationParam(
            ...
            defaultValue = "json",
            description =
                    "The file format to be used for representing the resulting FHIR ValueSets { json, xml } (default json)")
    ...
    @OperationParam(
            ...
            description =
                    "The directory path to which the generated FHIR ValueSet resources should be written (default src/main/resources/org/opencds/cqf/tooling/terminology/output)")
    ...
    public void execute() {
        ...
        generateValueSets(config)
                .forEach(vs -> IOUtils.writeResource(vs, outputPath, IOUtils.Encoding.parse(encoding), fhirContext));
    }
    ...
    public List<IBaseResource> generateValueSets(Config config) {
        ...
        if (config.getPathToIgResource() != null) {
            ...
            ImplementationGuide ig =
                    (ImplementationGuide) ResourceAndTypeConverter.convertToR5Resource(fhirContext, igResource);
        ...
        for (var valueSet : config.getValueSets()) {
            ...
            if (valueSet.getHierarchy() != null) {
                ...
                vs = (ValueSet)
                        ResourceAndTypeConverter.convertToR5Resource(fhirContext, hierarchyProcessor.getValueSet());
            } else if (valueSet.getRulesText() != null) {
                prepareRxMixWorkflowProcessor(valueSet.getRulesText());
                vs = (ValueSet)
                        ResourceAndTypeConverter.convertToR5Resource(fhirContext, rxMixWorkflowProcessor.getValueSet());
            } else if (valueSet.getExpand() != null) {
                prepareFhirTxExpansion(valueSet.getExpand());
                vs = (ValueSet) ResourceAndTypeConverter.convertToR5Resource(
                        fhirContext,
                        fhirTxExpansion.expandValueSet(
    ...
    public ValueSet updateValueSet(
            ValueSet vsToUpdate, Config config, Config.ValueSets configMetaData, CommonMetaData commonMetaData) {
        ...
        updatedValueSet.setVersion(
                configMetaData.getVersion() == null && commonMetaData != null
                        ? commonMetaData.getVersion()
                        : configMetaData.getVersion());
        updatedValueSet.setName(configMetaData.getName());
        updatedValueSet.setTitle(configMetaData.getTitle());
        updatedValueSet.setStatus(
                configMetaData.getStatus() == null
                        ? null
                        : Enumerations.PublicationStatus.fromCode(configMetaData.getStatus()));
        updatedValueSet.setExperimental(configMetaData.getExperimental());
        updatedValueSet.setDate(configMetaData.getDate());
        updatedValueSet.setPublisher(
                configMetaData.getPublisher() == null && commonMetaData != null
                        ? commonMetaData.getPublisher()
                        : configMetaData.getPublisher());
        updatedValueSet.setDescription(configMetaData.getDescription());
        updatedValueSet.setJurisdiction(
                configMetaData.getJurisdiction() == null && commonMetaData != null
                        ? commonMetaData.getJurisdiction()
                        : resolveJurisdiction(configMetaData.getJurisdiction()));
        updatedValueSet.setPurpose(configMetaData.getPurpose());
        updatedValueSet.setCopyright(
                configMetaData.getCopyright() == null && commonMetaData != null
                        ? commonMetaData.getCopyright()
                        : configMetaData.getCopyright());
        if (configMetaData.getProfiles() != null) {
            configMetaData
                    .getProfiles()
                    .forEach(profile -> updatedValueSet.getMeta().addProfile(profile));
        ...
        if (config != null && config.getAuthor() != null) {
            updatedValueSet.addExtension(
                    Terminology.VS_AUTHOR_EXT_URL,
                    new ContactDetail()
                            .setName(config.getAuthor().getName())
                            .setTelecom(Collections.singletonList(new ContactPoint()
                                    .setSystem(ContactPoint.ContactPointSystem.fromCode(
                                            config.getAuthor().getContactType()))
                                    .setValue(config.getAuthor().contactValue))));
        ...
        if (configMetaData.getClinicalFocus() != null) {
            updatedValueSet.addExtension(
                    Terminology.CLINICAL_FOCUS_EXT_URL, new StringType(configMetaData.getClinicalFocus()));
        }
        if (configMetaData.getDataElementScope() != null) {
            updatedValueSet.addExtension(
                    Terminology.DATA_ELEMENT_SCOPE_EXT_URL, new StringType(configMetaData.getDataElementScope()));
        }
        if (configMetaData.getInclusionCriteria() != null) {
            updatedValueSet.addExtension(
                    Terminology.VS_INCLUSION_CRITERIA_EXT_URL, new StringType(configMetaData.getInclusionCriteria()));
        }
        if (configMetaData.getExclusionCriteria() != null) {
            updatedValueSet.addExtension(
                    Terminology.VS_EXCLUSION_CRITERIA_EXT_URL, new StringType(configMetaData.getExclusionCriteria()));
        }
        if (configMetaData.getUsageWarning() != null) {
            updatedValueSet.addExtension(
                    Terminology.USAGE_WARNING_EXT_URL, new StringType(configMetaData.getUsageWarning()));
        }
        ...
        for (var compose : vs.getCompose().getInclude()) {
            for (var concept : compose.getConcept()) {
                codes.add(new Coding()
                        .setSystem(compose.getSystem())
                        .setCode(concept.getCode())
                        .setDisplay(concept.getDisplay())
                        .setVersion(compose.getVersion()));
            }
        }
        return codes;
    }
    ...
    static class CommonMetaData {
        ...
        public CommonMetaData(ImplementationGuide ig, Config.Author author) {
            ...
            if (author != null) {
                this.author = new ContactDetail()
                        .setName(author.getName())
                        .setTelecom(Collections.singletonList(new ContactPoint()
                                .setSystem(ContactPoint.ContactPointSystem.fromCode(author.getContactType()))
