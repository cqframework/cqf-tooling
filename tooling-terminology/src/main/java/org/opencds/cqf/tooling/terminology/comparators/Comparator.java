...
abstract class Comparator {
    ...
    protected void compareContacts(
            Set<Map<String, String>> fieldsWithErrors,
            List<ContactDetail> termContacts,
            List<ContactDetail> truthContacts) {
                ...
                termContactMap.forEach((termContactName, termContactPoints) -> {
                    ...
                    if (termContactPoints != null && truthContactPoints != null) {
                        ...
                        if (termContactPoints != null) {
                            contactFailure.put(
                                    "Contact",
                                    "This server's contact point has values and the matching IG contact point does not."
                                            + newLine);
                            fieldsWithErrors.add(contactFailure);
                        } else {
                            contactFailure.put(
                                    "Contact",
                                    "This server's contact point does not have values and the matching IG contact point does."
                                            + newLine);
                            fieldsWithErrors.add(contactFailure);
                        }
                    }
    ...
    private boolean compareContactPoints(
            Map<String, ContactPoint> termContactPoints,
            Map<String, ContactPoint> truthContactPoints,
            Map<String, String> contactPointFailure) {
        ...
        termContactPoints.forEach((termCPValue, termCP) -> {
            ...
            if (truthCP != null) {
                if (termCP.getSystem() != null && !termCP.getSystem().equals(truthCP.getSystem())) {
                    ...
                    contactPointFailure.put(
                            "ContactPoint",
                            "The server's contact point system with the value of \"" + termCP.getSystem()
                                    + "\" does not match the IG's contact point system of \"" + truthCP.getSystem()
                                    + "\"." + newLine);
                } else if (truthCP.getSystem() != null && termCP.getSystem() == null) {
                    contactPointsMatch.set(false);
                    contactPointFailure.put(
                            "ContactPoint",
                            "The IG's contact point system with the value of \"" + truthCP.getSystem()
                                    + "\" does not match the server's null value." + newLine);
                }
                if (termCP.getUse() != null && !termCP.getUse().equals(truthCP.getUse())) {
                    contactPointsMatch.set(false);
                    contactPointFailure.put(
                            "ContactPoint",
                            "The server's contact point use with the value of \"" + termCP.getUse()
                                    + "\" does not match the IG's contact point system of \"" + truthCP.getUse() + "\"."
                                    + newLine);
                } else if (truthCP.getUse() != null && termCP.getUse() == null) {
                    contactPointsMatch.set(false);
                    contactPointFailure.put(
                            "ContactPoint",
                            "The IG's contact point system with the value of \"" + truthCP.getUse()
                                    + "\" does not match the server's null value." + newLine);
                }
                if (termCP.getRank() != truthCP.getRank()) {
                    ...
                    contactPointFailure.put(
                            "ContactPoint",
                            "The server's contact point rank with the value of \"" + termCP.getRank()
                                    + "\" does not match the IG's contact point system of \"" + truthCP.getRank()
                                    + "\"." + newLine);
                }
                ...
                contactPointFailure.put(
                        "ContactPoint",
                        "The server's contact point with the value of \"" + termCPValue
                                + "\" does not exist in the IG's contact points." + newLine);
            }
        });
    ...
    private boolean comparePeriods(Period termPeriod, Period truthPeriod, Map<String, String> contactPointFailure) {
        ...
        if (termPeriod.getStart() != null && truthPeriod.getStart() != null) {
            if (!termPeriod.getStart().equals(truthPeriod.getStart())) {
                contactPointFailure.put(
                        "ContactPointPeriod",
                        "The server's contact point period with the start value of \"" + termPeriod.getStart()
                                + "\" does not match in the IG's contact point period start of \""
                                + truthPeriod.getStart() + "\"." + newLine);
                periodsMatch = false;
            }
        } else if (termPeriod.getStart() != null) {
            contactPointFailure.put(
                    "ContactPointPeriod",
                    "The server's contact point period start value of \"" + termPeriod.getStart()
                            + "\" does not match the IG's contact point period start of \"null\"." + newLine);
            periodsMatch = false;
        } else {
            contactPointFailure.put(
                    "ContactPointPeriod",
                    "The IG's contact point period start value of \"" + truthPeriod.getStart()
                            + "\" does not match the server's contact point period start of \"null\"." + newLine);
            periodsMatch = false;
        }
        if (termPeriod.getEnd() != null && truthPeriod.getEnd() != null) {
            if (!termPeriod.getEnd().equals(truthPeriod.getEnd())) {
                contactPointFailure.put(
                        "ContactPointPeriod",
                        "The server's contact point period with the end value of \"" + termPeriod.getEnd()
                                + "\" does not match in the IG's contact point period end of \"" + truthPeriod.getEnd()
                                + "\"." + newLine);
                periodsMatch = false;
            }
            ...
            contactPointFailure.put(
                    "ContactPointPeriod",
                    "The server's contact point period end value of \"" + termPeriod.getEnd()
                            + "\" does not match the IG's contact point period end of \"null\"." + newLine);
            periodsMatch = false;
        } else {
            contactPointFailure.put(
                    "ContactPointPeriod",
                    "The IG's contact point period end value of \"" + truthPeriod.getEnd()
                            + "\" does not match the server's contact point period end of \"null\"." + newLine);
            periodsMatch = false;
        }
        if (termPeriod.getId() != null && truthPeriod.getId() != null) {
            if (!termPeriod.getId().equals(truthPeriod.getId())) {
                contactPointFailure.put(
                        "ContactPointPeriod",
                        "The server's contact point period with the id value of \"" + termPeriod.getId()
                                + "\" does not match in the IG's contact point period id of \"" + truthPeriod.getId()
                                + "\"." + newLine);
                periodsMatch = false;
            }
            ...
            contactPointFailure.put(
                    "ContactPointPeriod",
                    "The server's contact point period id value of \"" + termPeriod.getId()
                            + "\" does not match the IG's contact point period id of \"null\"." + newLine);
            periodsMatch = false;
        } else {
            contactPointFailure.put(
                    "ContactPointPeriod",
                    "The IG's contact point period id value of \"" + truthPeriod.getId()
                            + "\" does not match the server's contact point period id of \"null\"." + newLine);
            periodsMatch = false;
        }
        return periodsMatch;
    }
