...
public class TemplateToValueSetGenerator extends Operation {
    ...
    private static final String SNOMEDCT = "http://snomed.info/sct";
    private static final String LOINC = "http://loinc.org";
    private static final String SNOMEDCT_BAD_URL = "SNOMED CT";
    private static final String LOINC_BAD_URL = "LOINC";
    ...
    private CPGMeta resolveCpgMeta(Sheet sheet) {
        ...
        while (rowIterator.hasNext()) {
            ...
            CellType cellType =
                    row.getCell(0, Row.MissingCellPolicy.CREATE_NULL_AS_BLANK).getCellType();
            String cellString = cellType.compareTo(CellType.STRING) == 0
                    ...
                    : String.valueOf(row.getCell(0, Row.MissingCellPolicy.CREATE_NULL_AS_BLANK)
                            .getNumericCellValue());
            ...
            switch (cellString) {
                ...
                case "title":
                    ...
                    if ((null != title && title.length() > 0)
                            && (null == meta.getId() || meta.getId().length() < 1)) {
                ...
                case "compose":
                    if (row.getCell(1, Row.MissingCellPolicy.CREATE_NULL_AS_BLANK)
                                    .getStringCellValue()
                                    .length()
                            > 1) meta.setCompose(SpreadsheetHelper.getCellAsStringNoReplacement(row.getCell(1)));
                    break;
                default:
                    break;
            }
     ...
     */
    private List<ValueSet> resolveValueSets(
            OrganizationalMetaData meta, Map<String, Integer> vsMap, Workbook workbook) {
        ...
        for (Map.Entry<String, Integer> entrySet : vsMap.entrySet()) {
            ...
            if (null != sheet) {
                ...
                try {
                    if (cpgMeta.getTitle().equals("only fill this out")) continue;
                ...
                resolveCodeList(
                        workbook.getSheet(entrySet.getKey().split("-")[0] + "-cl"), vs, meta.getSnomedVersion());
    ...
    private void resolveCodeList(Sheet sheet, ValueSet vs, String snomedVersion) {
        ...
        while (rowIterator.hasNext()) {
            ...
            if (code.equals("expansion")) {
                ...
                system = SpreadsheetHelper.getCellAsString(row.getCell(3, Row.MissingCellPolicy.CREATE_NULL_AS_BLANK))
                                == null
                        ? system
                        : SpreadsheetHelper.getCellAsString(row.getCell(3));

                if (system == null) throw new RuntimeException("A system must be specified in the code list");
    ...
    private void output(List<ValueSet> valueSets) {
        for (ValueSet valueSet : valueSets) {
            String prefixedOutputPath =
                    String.format("%s/%s%s.%s", getOutputPath(), outputPrefix, valueSet.getName(), encoding);
