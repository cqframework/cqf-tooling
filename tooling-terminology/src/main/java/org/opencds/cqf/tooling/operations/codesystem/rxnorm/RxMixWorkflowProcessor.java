...
public class RxMixWorkflowProcessor implements ExecutableOperation {
    ...
    @OperationParam(
            ...
            required = true,
            description =
                    "The workflow library expressed in XML that identifies that API functions needed to produce the desired output.")
    ...
    @OperationParam(
            ...
            required = true,
            description =
                    "The input values needed to run the workflow as a comma-delimited list of strings if multiple inputs are needed.")
    private String input;

    private List<String> inputs;

    @OperationParam(
            alias = {"includefilter", "if"},
            setter = "setIncludeFilter",
            description =
                    "The filter(s) that must be present within the RXCUI names for inclusion in the final result. Provide a comma-delimited list of strings for multiple filters.")
    private String includeFilter;

    private List<String> includeFilters;

    @OperationParam(
            alias = {"excludefilter", "ef"},
            setter = "setExcludeFilter",
            description =
                    "The filter(s) that must not be present within the RXCUI names for inclusion in the final result. Provide a comma-delimited list of strings for multiple filters.")
    ...
    @OperationParam(
            ...
            defaultValue = "json",
            description =
                    "The file format to be used for representing the resulting FHIR ValueSet { json, xml } (default json)")
    ...
    @OperationParam(
            ...
            description =
                    "The directory path to which the generated FHIR ValueSet resource should be written (default src/main/resources/org/opencds/cqf/tooling/terminology/output)")
    private String outputPath;

    private final LocalDate date = LocalDate.now();
    private String rxNormVersion = String.format(
            "%s-%s", date.getYear(), date.getMonthValue() < 10 ? "0" + date.getMonthValue() : date.getMonthValue());
    ...
    private void populateValueSet(String rawResponse, ValueSet vs) {
        if (!vs.getCompose().hasInclude()) {
            vs.getCompose()
                    .addInclude()
                    .setSystem(Terminology.RXNORM_SYSTEM_URL)
                    .setVersion(rxNormVersion);
        }
    ...
    public List<String> getIncludeFilters() {
        if (includeFilters == null && includeFilter != null) {
            includeFilters =
                    Arrays.stream(includeFilter.split(",")).map(String::trim).collect(Collectors.toList());
        }
    ...
    public List<String> getExcludeFilters() {
        if (excludeFilters == null && excludeFilter != null) {
            excludeFilters =
                    Arrays.stream(excludeFilter.split(",")).map(String::trim).collect(Collectors.toList());
        }
