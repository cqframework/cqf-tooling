...
public class HierarchyProcessor implements ExecutableOperation {
    ...
    @OperationParam(
            ...
            required = true,
            description =
                    "The expression that provides an alternative definition of the content of the value set in some form that is not computable - e.g. instructions that could only be followed by a human.")
    ...
    @OperationParam(
            ...
            setter = "setId",
            description =
                    "The narrative to populate the rulesText extension. If not present, the query will be used instead.")
    private String narrative;

    @OperationParam(
            alias = {"username", "user"},
            setter = "setUsername",
            defaultValue = "cschuler",
    ...
    private final List<String> validLoincVersions =
            Arrays.asList("2.69", "2.70", "2.71", "2.72", "2.73", "2.74", "2.75", "2.76");
    ...
    private ValueSet processValueSet() {
        ...
        valueSet.addExtension(
                Terminology.RULES_TEXT_EXT_URL, new StringType(narrative == null ? defaultQuery() : narrative));
        ...
        try (CloseableHttpClient queryClient = HttpClients.createDefault()) {
            ...
            for (var obj : arr) {
                if (obj.isJsonObject()) {
                    ...
                    if (jsonObj.has("IsLoinc")
                            && jsonObj.getAsJsonPrimitive("IsLoinc").getAsBoolean()) {
                        ...
                        valueSet.getExpansion()
                                .addContains()
                                .setVersion(loincVersion)
                                .setSystem(Terminology.LOINC_SYSTEM_URL)
                                .setCode(code)
                                .setDisplay(display);
                    }
                }
    ...
    private String defaultQuery() {
        return String.format(
                "Step 1: go to https://loinc.org/tree/%nStep 2: use query %s%nStep 3: export the results to CSV%nStep 4: Filter results by properties defined in query",
                query);
    }
    ...
    public void setLoincVersion(String loincVersion) {
        if (validLoincVersions.contains(loincVersion)) {
            ...
            logger.warn(
                    "Provided LOINC version {} is not supported. Valid versions include: {}",
                    loincVersion,
                    validLoincVersions);
        }
    }
