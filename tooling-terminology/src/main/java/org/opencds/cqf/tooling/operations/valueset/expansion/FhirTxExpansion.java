...
public class FhirTxExpansion implements ExecutableOperation {
    ...
    @OperationParam(
            alias = {"pathtovalueset", "ptvs"},
            setter = "setPathToValueSet",
            required = true,
    ...
    @OperationParam(
            ...
            defaultValue = "http://tx.fhir.org/r4",
    ...
    public void execute() {
        ...
        if (Files.isDirectory(Paths.get(pathToValueSet))) {
            for (IBaseResource resource :
                    IOUtils.readResources(IOUtils.getFilePaths(pathToValueSet, true), fhirContext)) {
