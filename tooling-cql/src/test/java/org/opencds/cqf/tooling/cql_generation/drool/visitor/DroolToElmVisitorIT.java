...
public class DroolToElmVisitorIT {
        ...
        for (String expression : expressions) {
            ...
            if (expression.matches(observationInterpretationRelatedExpressions)) {
                ...
                if (expression.equals(
                    ...
                    try {
                        EvaluationResult evaluationResult = engine.evaluate(
                                new VersionedIdentifier()
                                        .withId("[]_Thrombocytopenia_UPDATED_TEMPLATE_5477e")
                                        .withVersion("4.0.1"),
                                Collections.singleton(expression));
                        Object result =
                                evaluationResult.forExpression(expression).value();
    ...
    public void test_ModelingK() throws IOException {
        ...
        String expressionListFilePath = "GeneratedCql14_922c0_Expression.txt";
        List<Object> results = runAllExpressionsFromFile(
                xmlFileName,
                new VersionedIdentifier().withId("GeneratedCql14_922c0").withVersion("4.0.1"),
                expressionListFilePath);
        results.forEach(object -> Assert.assertTrue((Boolean) object));
    }
    ...
    public void test_ModelingLAndD() throws IOException {
        ...
        List<Object> results = runAllExpressionsFromFile(
                xmlFileName,
                new VersionedIdentifier()
                        .withId("_Medications_Administered_2764c")
                        .withVersion("4.0.1"),
                expressionListFilePath);
        results.forEach(object -> Assert.assertTrue((Boolean) object));
    }
    ...
    public void test_ModelingC() throws IOException {
        ...
        List<Object> results = runAllExpressionsFromFile(
                xmlFileName,
                new VersionedIdentifier().withId("GeneratedCql136_89fdc").withVersion("4.0.1"),
                expressionListFilePath);
        results.forEach(object -> Assert.assertTrue((Boolean) object));
    }
    ...
    public void test_ModelingN() throws IOException {
        ...
        List<Object> results = runAllExpressionsFromFile(
                xmlFileName,
                new VersionedIdentifier().withId("Patient_is_deceased_84755").withVersion("4.0.1"),
                expressionListFilePath);
        results.forEach(object -> Assert.assertTrue((Boolean) object));
    }
    ...
    public void other() throws IOException {
        ...
        List<Object> results = runAllExpressionsFromFile(
                xmlFileName,
                new VersionedIdentifier().withId("Abdominal_Cramps_314c4").withVersion("4.0.1"),
                expressionListFilePath);
        results.forEach(object -> Assert.assertTrue((Boolean) object));
    }
    ...
    public void otherOther() throws IOException {
        ...
        List<Object> results = runAllExpressionsFromFile(
                xmlFileName,
                new VersionedIdentifier().withId("Anorexia_baace").withVersion("4.0.1"),
                expressionListFilePath);
        results.forEach(object -> Assert.assertTrue((Boolean) object));
    }

    private List<Object> runAllExpressionsFromFile(
            String xmlFileName, VersionedIdentifier versionedIdentifier, String expressionListFilePath)
            throws IOException {
        setup(xmlFileName);
    ...
    public void setup(String libraryPath) throws IOException {
        ...
        IBaseBundle bundle = fhirContext
                .newJsonParser()
                .parseResource(Bundle.class, DroolToElmVisitorIT.class.getResourceAsStream("concepts_full.json"));
        Environment environment = null; // TODO: Use a BundleRepositoryProvider?
        // new Environment(libraryManager, getDataProviders(fhirContext), new BundleTerminologyProvider(fhirContext,
        // bundle));
    ...
    private Map<String, DataProvider> getDataProviders(FhirContext fhirContext) {
        ...
        IBaseBundle dataBundle = fhirContext
                .newJsonParser()
                .parseResource(Bundle.class, DroolToElmVisitorIT.class.getResourceAsStream("AllResources.json"));

        // TODO: Use a BundleRepositoryProvider?
        // DataProvider dataProvider = new CompositeDataProvider(new CachingModelResolverDecorator(new
        // R4FhirModelResolver()),
    ...
    public static List<String> readFileInList(String fileName) throws IOException {
        ...
        BufferedReader reader = new BufferedReader(
                new InputStreamReader(Objects.requireNonNull(DroolToElmVisitorIT.class.getResourceAsStream(fileName))));
        while (reader.ready()) {
