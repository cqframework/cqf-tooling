...
public abstract class VmrToModelElmBuilder {
    ...
    public VmrToModelElmBuilder(
            String modelIdentifier,
            String modelVersion,
            String modelUri,
            LibrarySourceProvider lsp,
    ...
    public abstract Expression resolveModeling(
    ...
    private Expression resolveOperator(
            LibraryBuilder libraryBuilder, String operator, Expression left, Expression right) {
        ...
        switch (operator) {
            case ">=": {
                if (left.getResultType() instanceof ListType) {
                    ...
                    AliasedQuerySource source =
                            of.createAliasedQuerySource().withAlias("Alias").withExpression(left);
                    sources.add(source);
            ...
            case "!=": {
                if (right instanceof ValueSetRef || right instanceof CodeRef) {
                    operatorExpression =
                            resolveNot(libraryBuilder, resolveMembership(libraryBuilder, "in", left, right));
    ...
    public Expression resolveCountQuery(
            LibraryBuilder libraryBuilder, Expression expression, Expression right, String operator) {
        logger.debug(markers.get("Modeling"), "Resolving Count Query");
        List<Element> elements = new ArrayList<Element>();
        List<AliasedQuerySource> sources = new ArrayList<AliasedQuerySource>();
        AliasedQuerySource source =
                of.createAliasedQuerySource().withAlias("Alias").withExpression(expression);
    ...
    private Expression resolveWhere(
        ...
        if (right instanceof CodeRef) {
            ...
            throw new RuntimeException("Unable to build predicate expression for: " + left.getResultType() + " and "
                    + right.getResultType());
        }
    }
    ...
    public Expression resolveMembership(
            LibraryBuilder libraryBuilder, String operator, Expression left, Expression right) {
        switch (operator) {
            case "in":
    ...
    public Expression resolveEquality(
            LibraryBuilder libraryBuilder, String operator, Expression left, Expression right) {
    ...
    public BinaryExpression resolveInequality(
            LibraryBuilder libraryBuilder, String operator, Expression left, Expression right) {
    ...
    public Retrieve resolveRetrieve(
            LibraryBuilder libraryBuilder,
            String resource,
                ...
                try {
                    ...
                    propertyException = new CqlSemanticException(
                            String.format(
                                    ...
                                    retrieve.getCodeProperty(), namedType.getName()),
    ...
    public Object resolveAliasedQuerySource(Expression querySource, String alias) {
        ...
        AliasedQuerySource source =
                of.createAliasedQuerySource().withExpression(querySource).withAlias(alias);
    ...
    public CodeRef resolveCode(
            LibraryBuilder libraryBuilder, String codeId, String codeName, String codeDisplay, CodeSystemRef csRef) {
        ...
        if (code == null) {
            ...
            code = of.createCodeDef()
                    .withAccessLevel(AccessModifier.PUBLIC)
                    .withCodeSystem(csRef)
                    .withDisplay(codeDisplay)
                    .withId(codeId)
                    .withName(codeName);
