...
public class LibraryConverter {
     ...
     */
    public Pair<VersionedIdentifier, ContextDef> adapt(
            ConditionDTO conditionDTO, VmrToModelElmBuilder modelBuilder, Integer libraryIndex) {
        ...
        if (conditionDTO.getCdsCodeDTO() != null) {
            ...
            throw new RuntimeException("Unable to infer library name for condition "
                    + conditionDTO.getUuid().toString());
        }
     ...
     */
    public Pair<VersionedIdentifier, ContextDef> adapt(
            ConditionCriteriaRelDTO conditionCriteriaRel, VmrToModelElmBuilder modelBuilder, Integer libraryIndex) {
        ...
        if (conditionCriteriaRel.getCriteriaDTO() != null) {
            ...
            header = header.concat(" "
                    + conditionCriteriaRel.getCriteriaDTO().getCriteriaType().toString());
        } else {
            logger.error(markers.get("Library"), "conditionCriteriaRel.getCriteriaDTO() was null.");
            throw new RuntimeException("Unable to infer library name for condition "
                    + conditionCriteriaRel.getUuid().toString());
        }
        logger.debug("Resolving context for library {}", libraryName);
        return resolveContext(libraryName, header, modelBuilder);
    }
    ...
    private Pair<VersionedIdentifier, ContextDef> resolveContext(
            String libraryName, String header, VmrToModelElmBuilder modelBuilder) {
        VersionedIdentifier versionedIdentifier =
                new VersionedIdentifier().withId(libraryName).withVersion("1.0.0");
