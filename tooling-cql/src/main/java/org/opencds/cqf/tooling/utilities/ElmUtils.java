    ...
    public static CompiledLibrary generateCompiledLibrary(Library library) {
        ...
        try {
            ...
            if (library.getStatements() != null && library.getStatements().getDef() != null) {
                // Sort the defs - this is needed because the translator uses the binary search algorithm to resolve
                // expressions
                List<ExpressionDef> expressionDefs =
                        new ArrayList<>(library.getStatements().getDef());
