...
public class CqlProcessor {
            ...
            for (Map.Entry<String, CqlSourceFileInformation> entry : fileMap.entrySet()) {
                if (filename.equalsIgnoreCase(entry.getKey())) {
                    logger.logDebugMessage(
                            ...
                            String.format(
                                    "File with a similar name but different casing was found. File found: '%s'",
                                    entry.getKey()));
                }
            }
            return null;
        ...
        if (cachedOptions == null) {
            if (hasMultipleBinaryPaths) {
                throw new RuntimeException(
                        "CqlProcessor has been used with multiple Cql paths, ambiguous options and manager");
            } else {
    ...
    private void translateFolder(String folder) {
        ...
        if (packages != null) {
            modelManager
                    .getModelInfoLoader()
                    .registerModelInfoProvider(new NpmModelInfoProvider(packages, reader, logger), true);
            libraryManager
                    .getLibrarySourceLoader()
                    .registerProvider(new NpmLibrarySourceProvider(packages, reader, logger));
        }
                ...
                if (p.name() != null
                    ...
                    if (libraryManager.getNamespaceManager().resolveNamespaceUri(ni.getName()) != null) {
                        logger.logMessage(String.format(
                                "Skipped loading namespace info for name %s because it is already registered",
                                ni.getName()));
                    } else if (libraryManager.getNamespaceManager().getNamespaceInfoFromUri(ni.getUri()) != null) {
                        logger.logMessage(String.format(
                                "Skipped loading namespace info for uri %s because it is already registered",
                                ni.getUri()));
                    } else {
    ...
    public static ValidationMessage exceptionToValidationMessage(File file, CqlCompilerException exception) {
        ...
        if (tb != null) {
            return new ValidationMessage(
                    ValidationMessage.Source.Publisher,
                    severityToIssueType(exception.getSeverity()),
                    tb.getStartLine(),
                    tb.getStartChar(),
                    tb.getLibrary().getId(),
    ...
    private void translateFile(LibraryManager libraryManager, File file, CqlCompilerOptions options) {
        ...
        try {
            ...
            List<CqlCompilerException> severeErrorList =
                    listBySeverity(translator.getErrors(), CqlCompilerException.ErrorSeverity.Error);
            ...
            if (!severeErrorList.isEmpty()) {
                ...
                result.getErrors()
                        .add(new ValidationMessage(
                                ...
                                file.getName(),
                                String.format(
                                        "CQL Processing failed with (%d) errors.",
                                        translator.getErrors().size()),
                                IssueSeverity.ERROR));
            } else {
                try {
                    ...
                    // Add the translated library to the library manager (NOTE: This should be a "cacheLibrary" call on
                    // the LibraryManager, available in 1.5.3+)
                    // Without this, the data requirements processor will try to load the current library, resulting in
                    // a re-translation
                    ...
                    org.hl7.fhir.r5.model.Library requirementsLibrary = drp.gatherDataRequirements(
                            libraryManager, translator.getTranslatedLibrary(), options, null, false, false);
                    ...
                    for (RelatedArtifact relatedArtifact : requirementsLibrary.getRelatedArtifact()) {
                        ...
                        if (relatedArtifact.hasResource()
                                && (relatedArtifact
                                                .getResource()
                                                .startsWith("http://hl7.org/fhir/Library/QICore-ModelInfo")
                                        || relatedArtifact
                                                .getResource()
                                                .startsWith("http://fhir.org/guides/cqf/common/Library/FHIR-ModelInfo")
                                        || relatedArtifact
                                                .getResource()
                                                .startsWith("http://hl7.org/fhir/Library/USCore-ModelInfo"))) {
                    ...
                    for (ValidationMessage paramMessage : drp.getValidationMessages()) {
                        result.getErrors()
                                .add(new ValidationMessage(
                                        paramMessage.getSource(),
                                        paramMessage.getType(),
                                        file.getName(),
                                        paramMessage.getMessage(),
                                        paramMessage.getLevel()));
                    }
                    ...
                    logger.logMessage(String.format(
                            "CQL Translation succeeded for file: '%s', but ELM generation failed with the following error: %s",
            ...
            result.getErrors()
                    .add(new ValidationMessage(
                            ...
                            IssueType.EXCEPTION,
                            file.getName(),
                            "CQL Processing failed with exception: " + e.getMessage(),
                            IssueSeverity.ERROR));
        }
    }
    ...
    private static List<CqlCompilerException> listBySeverity(
            List<CqlCompilerException> errors, CqlCompilerException.ErrorSeverity errorSeverity) {
    ...
    public static String buildStatusMessage(
            List<CqlCompilerException> errors, String resourceName, boolean verboseMessaging) {
    ...
    public static String buildStatusMessage(
        ...
        return (withStatusIndicator ? statusIndicator : "") + "CQL Processing of " + resourceName + statusIndicatorMinor
                + " with " + errorsStatus + ", " + warningStatus + ", and " + infoStatus
                + (verboseMessaging ? ": " + delimiter + fullSortedListMsg : "");
    }
