...
public class ResourceAndTypeConverter {
        ...
        switch (fhirContext.getVersion().getVersion()) {
            case R5:
                if (type instanceof org.hl7.fhir.dstu3.model.Type) {
                    ...
                    throw new UnsupportedOperationException("Conversion to R5 type not supported for "
            ...
            case R4:
                if (type instanceof org.hl7.fhir.r5.model.DataType) {
                    ...
                    throw new UnsupportedOperationException("Conversion to R4 type not supported for "
                            + type.getClass().toString());
                }
            case DSTU3:
                if (type instanceof org.hl7.fhir.r5.model.DataType) {
                    ...
                    throw new UnsupportedOperationException("Conversion to DSTU3 type not supported for "
                            + type.getClass().toString());
                }
            default:
                throw new UnsupportedOperationException("Conversion not supported for types using version "
    ...
    public static org.hl7.fhir.r5.model.Resource convertToR5Resource(FhirContext fhirContext, IBaseResource resource) {
        switch (fhirContext.getVersion().getVersion()) {
            ...
            case R4:
                return r4ToR5Resource(resource);
            case DSTU3:
                return stu3ToR5Resource(resource);
            default:
                throw new UnsupportedOperationException("Conversion to R5 not supported for resources using version "
                        + fhirContext.getVersion().getVersion().getFhirVersionString());
        }
    }

    public static IBaseResource convertFromR5Resource(
            FhirContext fhirContext, org.hl7.fhir.r5.model.Resource resource) {
        switch (fhirContext.getVersion().getVersion()) {
            case R5:
                return resource;
            case R4:
                return r5ToR4Resource(resource);
            case DSTU3:
                return r5ToStu3Resource(resource);
            default:
                throw new UnsupportedOperationException("Conversion from R5 not supported for resources using version "
                        + fhirContext.getVersion().getVersion().getFhirVersionString());
        }
    }
