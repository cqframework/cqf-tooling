...
public class PackageMeasures {
    ...
    public PackageMeasures(
        ...
        var measureResourcePaths = ResourceDiscovery.getMeasurePaths(fhirContext);
        if (fhirContext.getVersion().getVersion() == FhirVersionEnum.R4) {
            measureResourcePaths.forEach(path -> new PackageMeasure(
                            igRoot,
                            fhirContext,
                            path,
                            includeDependencies,
            ...
            throw new UnsupportedOperationException(
                    "Package operation for Measure resources is not supported for FHIR version: "
