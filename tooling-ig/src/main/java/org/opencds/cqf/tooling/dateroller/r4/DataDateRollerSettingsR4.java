...
public class DataDateRollerSettingsR4 {
    private LocalDate lastDateUpdated;
    private Float durationLength;
    private String durationUnitCode;

    public LocalDate getLastDateUpdated() {
        return lastDateUpdated;
    }

    public void setLastDateUpdated(LocalDate lastDateUpdated) {
        this.lastDateUpdated = lastDateUpdated;
    }

    public Float getDurationLength() {
    ...
    public boolean populateDataDateRollerSettings(Resource resource) {
        ...
        for (Base extValue : extValues) {
            ...
            if (url.equalsIgnoreCase("http://fhir.org/guides/cdc/opioid-cds/StructureDefinition/dataDateRoller")) {
                extensionFound = true;
                String firstExtensionUrlValue = extValue.getChildByName("extension")
                        .getValues()
                        .get(0)
                        .getChildByName("url")
                        .getValues()
                        .get(0)
                        .toString();
                String secondExtensionUrlValue = extValue.getChildByName("extension")
                        .getValues()
                        .get(1)
                        .getChildByName("url")
                        .getValues()
                ...
                if (null != firstExtensionUrlValue && null != secondExtensionUrlValue) {
                    ...
                    throw new IllegalArgumentException(
                            "Extension http://fhir.org/guides/cdc/opioid-cds/StructureDefinition/dataDateRoller is not formatted correctly.");
                }
            }
        }
    ...
    private void getDataDateRollerSettings(Base extValue, String extensionUrlValue, int extensionPosition) {
        if (extensionUrlValue.contains("dateLastUpdated")) {
            getLastUpdatedSettings(extValue.getChildByName("extension")
                    .getValues()
                    .get(extensionPosition)
                    .getNamedProperty("value")
                    .getValues()
                    .get(0));
        }
        if (extensionUrlValue.contains("frequency")) {
            getFrequencySettings(extValue.getChildByName("extension")
                    .getValues()
                    .get(extensionPosition)
                    .getNamedProperty("value")
                    .getValues()
    ...
    private void getFrequencySettings(Base frequency) {
        this.setDurationUnitCode(
                frequency.getNamedProperty("code").getValues().get(0).toString());
        String codeValue =
                frequency.getNamedProperty("value").getValues().get(0).toString();
        this.setDurationLength(
                Float.parseFloat(codeValue.substring(codeValue.indexOf("[") + 1, codeValue.indexOf("]"))));
    }

    private void getLastUpdatedSettings(Base lastUpdated) {
        String lastUpdatedStringDate = lastUpdated.toString();
        lastUpdatedStringDate = lastUpdatedStringDate.substring(
                lastUpdatedStringDate.indexOf("[") + 1, lastUpdatedStringDate.indexOf("]"));
