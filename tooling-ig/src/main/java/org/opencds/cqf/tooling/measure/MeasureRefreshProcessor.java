...
public class MeasureRefreshProcessor {
    ...
    public Measure refreshMeasure(
        ...
        clearMeasureExtensions(
                measureToUse, "http://hl7.org/fhir/us/cqfmeasures/StructureDefinition/cqfm-dataRequirement");
        clearMeasureExtensions(
                measureToUse, "http://hl7.org/fhir/us/cqfmeasures/StructureDefinition/cqfm-directReferenceCode");
        clearMeasureExtensions(
                measureToUse, "http://hl7.org/fhir/us/cqfmeasures/StructureDefinition/cqfm-logicDefinition");
        clearMeasureExtensions(
                measureToUse, "http://hl7.org/fhir/us/cqfmeasures/StructureDefinition/cqfm-effectiveDataRequirements");
        ...
        Library moduleDefinitionLibrary =
                getModuleDefinitionLibrary(measureToUse, libraryManager, compiledLibrary, options);
        ...
        if (moduleDefinitionLibrary.hasRelatedArtifact()) {
            for (int i = moduleDefinitionLibrary.getRelatedArtifact().size() - 1; i >= 0; i--) {
                RelatedArtifact relatedArtifact =
                        moduleDefinitionLibrary.getRelatedArtifact().get(i);
    ...
    private Library getModuleDefinitionLibrary(
            Measure measureToUse,
            LibraryManager libraryManager,
            CompiledLibrary compiledLibrary,
            CqlCompilerOptions options) {
    ...
    private void setPopulationDataRequirements(
            Measure measureToUse,
            LibraryManager libraryManager,
            CompiledLibrary compiledLibrary,
            CqlCompilerOptions options) {
        ...
        measureToUse
                .getGroup()
                .forEach(groupMember -> groupMember.getPopulation().forEach(population -> {
                    if (population.hasId()) { // Requirement for computable measures
                        var popMDL = dqReqTrans.gatherDataRequirements(
                                libraryManager,
                                compiledLibrary,
                                options,
                                Collections.singleton(population.getCriteria().getExpression()),
                                false);
                        ...
                        population
                                .getExtension()
                                .removeAll(population.getExtensionsByUrl(
                                        CrmiConstants.EFFECTIVE_DATA_REQUIREMENTS_EXT_URL));
                        population.addExtension(
                                CrmiConstants.EFFECTIVE_DATA_REQUIREMENTS_EXT_URL, new CanonicalType("#" + mdlID));
                    }
                }));
    }
    ...
    private void setMeta(Measure measureToUse, Library moduleDefinitionLibrary) {
        ...
        for (CanonicalType canonical : measureToUse.getMeta().getProfile()) {
            if ("http://hl7.org/fhir/us/cqfmeasures/StructureDefinition/computable-measure-cqfm"
                    .equals(canonical.getValue())) {
        ...
        if (!hasProfileMarker) {
            measureToUse
                    .getMeta()
                    .addProfile("http://hl7.org/fhir/us/cqfmeasures/StructureDefinition/computable-measure-cqfm");
        }
    }
}
