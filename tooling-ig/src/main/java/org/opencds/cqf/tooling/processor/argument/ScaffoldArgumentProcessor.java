...
public class ScaffoldArgumentProcessor {
    ...
    public OptionParser build() {
        ...
        OptionSpecBuilder igPathBuilder =
                parser.acceptsAll(asList(IG_PATH_OPTIONS), "Limited to a single version of FHIR.");
        ...
        OptionSpecBuilder igOutputEncodingBuilder = parser.acceptsAll(
                asList(IG_OUTPUT_ENCODING), "JSON|XML - If omitted, output will be generated using JSON encoding.");
        OptionSpecBuilder nameOfResourceToCreateBuilder = parser.acceptsAll(
                asList(NAME_OF_RESOURCE_CREATE_OPTIONS),
                "Use multiple times to define names of resources that should be created.");
        OptionSpecBuilder softwareSystemBuilder =
                parser.acceptsAll(asList(SOFTWARE_SYSTEM_OPTIONS), "Use multiple times to define multiple software.");
        ...
        OptionSpec<String> resourceNamesToCreate =
                nameOfResourceToCreateBuilder.withOptionalArg().describedAs("Name to be used for resources created");
        OptionSpec<String> softwareSystem =
                softwareSystemBuilder.withOptionalArg().describedAs("Software System name and version");
