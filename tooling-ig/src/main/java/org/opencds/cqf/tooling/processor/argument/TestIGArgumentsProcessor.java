...
public class TestIGArgumentsProcessor {
    ...
    public OptionParser build() {
        ...
        //        OptionSpecBuilder igCanonicalBaseBuilder = parser.acceptsAll(asList(IG_CANONICAL_BASE),"resource
        // canonical base");
        OptionSpecBuilder fhirVersionBuilder =
                parser.acceptsAll(asList(FHIR_VERSION_OPTIONS), "Limited to a single version of FHIR.");
        OptionSpecBuilder testCasesPathBuilder =
                parser.acceptsAll(asList(TEST_CASES_PATH_OPTIONS), "Path to test cases");
        ...
        OptionSpec<String> igPath =
                igPathBuilder.withOptionalArg().describedAs("Path to the IG, relative to the root directory");
        ...
        OptionSpec<String> testCasesPath =
                testCasesPathBuilder.withRequiredArg().describedAs("path to the test cases");
        OptionSpec<String> fhirServerUri =
                fhirUriBuilder.withRequiredArg().describedAs("uri for fhir server to test on");
