...
public class DataDateRollerUtilsDstu3 {
    private static Logger logger = LoggerFactory.getLogger(ResourceDataDateRoller.class);

    public static LocalDate rollDate(LocalDate dateToRoll, DataDateRollerSettingsDstu3 dataDateRollerSettings) {
        return dateToRoll.plusDays(LocalDate.now().getLong(ChronoField.EPOCH_DAY)
                - dataDateRollerSettings.getLastDateUpdated().getLong(ChronoField.EPOCH_DAY));
    }

    public static boolean isCurrentDateGreaterThanInterval(DataDateRollerSettingsDstu3 dataDateRollerSettings) {
        LocalDateTime now = LocalDateTime.now();
        LocalDateTime lastUpdated = LocalDate.parse(
                        dataDateRollerSettings.getLastDateUpdated().toString())
                .atStartOfDay();
        ...
        switch (dataDateRollerSettings.getDurationUnitCode()) {
            ...
            default:
                throw new IllegalArgumentException(
                        "The code in the Extension http://fhir.org/guides/cdc/opioid-cds/StructureDefinition/dataDateRoller for duration is incorrect. Must be {d, w, m, or y}");
        }
        return canUpdateDates;
    }

    public static LocalDate getOldDateFromResource(Resource resource, Field field) {
        String strDateToRoll =
                resource.getNamedProperty(field.getName()).getValues().get(0).toString();
    ...
    public static LocalDate getLocalDateFromPeriod(Property period, String periodPosition) {
        DateTimeType startTimeType = getDateTimeTypeFromPeriod(period, periodPosition);
        return DataDateRollerUtils.stringDateFromResourceToLocalDate(
                getDateTimeTypeFromPeriod(period, periodPosition).toString());
    }

    public static DateTimeType getDateTimeTypeFromPeriod(Property period, String periodPosition) {
        return (DateTimeType) period.getValues()
                .get(0)
                .getChildByName(periodPosition)
                .getValues()
                .get(0);
    }
        ...
        for (Base extValue : extValues) {
            ...
            if (url.equalsIgnoreCase("http://fhir.org/guides/cdc/opioid-cds/StructureDefinition/dataDateRoller")) {
                for (Base base : extValue.getChildByName("extension").getValues()) {
                    String extensionUrlValue =
                            base.getChildByName("url").getValues().get(0).toString();
