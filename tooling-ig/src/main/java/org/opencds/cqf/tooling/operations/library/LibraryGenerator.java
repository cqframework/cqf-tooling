...
public class LibraryGenerator implements ExecutableOperation {
    ...
    @OperationParam(
            alias = {"ptcql", "pathtocqlcontent"},
            setter = "setPathToCqlContent",
            required = true,
            description =
                    "Path to the directory or file containing the CQL content to be transformed into FHIR Library resources (required)")
    ...
    @OperationParam(
            ...
            description =
                    "The directory path to which the generated FHIR Library resources should be written (default src/main/resources/org/opencds/cqf/tooling/library/output)")
    ...
    public void execute() {
        ...
        LibrarySourceProvider librarySourceProvider = new DefaultLibrarySourceProvider(
                cqlContent.isDirectory()
                        ? cqlContent.toPath()
                        : cqlContent.getParentFile().toPath());
        ...
        if (cqlFile.getAbsolutePath().endsWith("cql")) {
            try {
                ...
                org.hl7.fhir.r5.model.Library dataReqLibrary = dataRequirementsProcessor.gatherDataRequirements(
                        ...
                        translatorOptions.getCqlCompilerOptions(),
                        null,
                        false);
                IOUtils.writeResource(
                        resolveFhirLibrary(translator, dataReqLibrary, IOUtils.getFileContent(cqlFile)),
                        outputPath,
    ...
    public IBaseResource resolveFhirLibrary(
            CqlTranslator cqlTranslator, org.hl7.fhir.r5.model.Library dataReqLibrary, String cql) {
        ...
        library.setId(nameToId(
                elmLibrary.getIdentifier().getId(), elmLibrary.getIdentifier().getVersion()));
