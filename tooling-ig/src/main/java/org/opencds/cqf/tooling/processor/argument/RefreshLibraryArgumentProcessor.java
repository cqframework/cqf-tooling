...
public class RefreshLibraryArgumentProcessor {
    ...
    public static final String[] Library_Output_DIRECTORY_OPTIONS = {
        "libraryOutput", "libraryOutputPath", "resourceOutputPath", "lop"
    };
    ...
    public OptionParser build() {
        ...
        OptionSpecBuilder igCanonicalBaseBuilder =
                parser.acceptsAll(asList(IG_CANONICAL_BASE), "resource canonical base");
        OptionSpecBuilder cqlPathBuilder =
                parser.acceptsAll(asList(CQL_PATH_OPTIONS), "Library will be created in the same folder as the cql");
        OptionSpecBuilder libraryPathBuilder = parser.acceptsAll(
                asList(Library_PATH_OPTIONS), "If omitted, the library will be created in the same folder as the cql");
        OptionSpecBuilder libraryOutputDirectoryBuilder = parser.acceptsAll(
                asList(Library_Output_DIRECTORY_OPTIONS),
                "If omitted, the libraries will overwrite any existing libraries");
        OptionSpecBuilder fhirVersionBuilder =
                parser.acceptsAll(asList(FHIR_VERSION_OPTIONS), "Limited to a single version of FHIR.");
        OptionSpecBuilder outputEncodingBuilder =
                parser.acceptsAll(asList(OUTPUT_ENCODING), "If omitted, output will be generated using JSON encoding.");
        OptionSpecBuilder shouldApplySoftwareSystemStampBuilder = parser.acceptsAll(
                asList(SOFTWARE_STAMP),
                "Indicates whether refreshed Library resources should be stamped with the 'cqf-tooling' stamp via the crmi-softwaresystem Extension.");

        OptionSpec<String> ini = iniBuilder.withOptionalArg().describedAs("IG ini file");
        OptionSpec<String> igCanonicalBasePath =
                igCanonicalBaseBuilder.withOptionalArg().describedAs("resource canonical base");
        ...
        OptionSpec<String> libraryOutputDirectoryPath = libraryOutputDirectoryBuilder
                .withOptionalArg()
        ...
        OptionSpec<String> outputEncoding =
                outputEncodingBuilder.withOptionalArg().describedAs("desired output encoding for resources");
