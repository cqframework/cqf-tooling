...
public class VmrToFhirArgumentProcessor {
    ...
    public OptionParser build() {
        ...
        OptionSpecBuilder encodingBuilder =
                parser.acceptsAll(asList(ENCODING_OPTIONS), "If omitted, encoding input will be expected to be xml.");
        OptionSpecBuilder inputFilePathBuilder =
                parser.acceptsAll(asList(INPUT_FILE_PATH_OPTIONS), "Must be a path to Vmr Data File.");
        ...
        OptionSpec<String> encoding = encodingBuilder
                .withOptionalArg()
                .describedAs("input encoding (as of now can only be xml)")
                .defaultsTo("xml");
        OptionSpec<String> inputFilePath =
                inputFilePathBuilder.withRequiredArg().describedAs("input vmr file path");
