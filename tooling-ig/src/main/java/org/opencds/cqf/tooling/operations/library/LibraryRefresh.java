...
public class LibraryRefresh implements ExecutableOperation {
    ...
    @OperationParam(
            alias = {"ptl", "pathtolibrary"},
            setter = "setPathToLibrary",
            required = true,
            description = "Path to the FHIR Library resource to refresh (required).")
    private String pathToLibrary;

    @OperationParam(
            alias = {"ptcql", "pathtocql"},
            setter = "setPathToCql",
            required = true,
            description =
                    "Path to the CQL content referenced or depended on by the FHIR Library resource to refresh (required).")
    ...
    public void execute() {
        ...
        try {
            ...
            libraryManager
                    ...
                    .registerProvider(new DefaultLibrarySourceProvider(Paths.get(pathToCql)));
    ...
    public IBaseResource refreshLibrary(IBaseResource libraryToRefresh) {
        Library r5LibraryToRefresh =
                (Library) ResourceAndTypeConverter.convertToR5Resource(fhirContext, libraryToRefresh);

        try {
            String cql = new String(libraryManager
                    .getLibrarySourceLoader()
                    .getLibrarySource(ResourceUtils.getIdentifier(libraryToRefresh, fhirContext))
                    .readAllBytes());
            ...
            Library drLibrary = drp.gatherDataRequirements(
                    libraryManager,
