...
public class GenerateCQLFromDroolArgumentProcessor {
    ...
    public static final String[] INPUT_FILE_PATH_OPTIONS = {
        "ip", "inputPath", "input-path", "ifp", "inputFilePath", "input-file-path", "input-filePath"
    };
    ...
    public OptionParser build() {
        ...
        OptionSpecBuilder outputBuilder = parser.acceptsAll(
                asList(OUTPUT_PATH_OPTIONS), "Will be created if file path does not currently exist.");
        OptionSpecBuilder encodingBuilder =
                parser.acceptsAll(asList(ENCODING_OPTIONS), "If omitted, encoding input will be expected to be json.");
        OptionSpecBuilder inputFilePathBuilder = parser.acceptsAll(
                asList(INPUT_FILE_PATH_OPTIONS), "Must be a path to encoded logic export required for cql generation.");
        OptionSpecBuilder fhirVersionBuilder =
                parser.acceptsAll(asList(FHIR_VERSION_OPTIONS), "If omitted, defualt version will be 4.0.0");
        OptionSpecBuilder cqlTypeBuilder =
                parser.acceptsAll(asList(CQLTYPES_OPTIONS), "If omitted, defualt granularity will be CONDITION");

        OptionSpec<String> outputPath =
                outputBuilder.withRequiredArg().describedAs("path to desired cql generation output");
        OptionSpec<String> encoding = encodingBuilder
                .withOptionalArg()
                .describedAs("input encoding (as of now can only be json)")
                .defaultsTo("json");
        OptionSpec<String> inputFilePath =
                inputFilePathBuilder.withRequiredArg().describedAs("input encoded file path");
        OptionSpec<String> fhirVersion =
                fhirVersionBuilder.withOptionalArg().describedAs("FHIR Model Version to map elm to.");
    ...
    public GenerateCQLFromDroolParameters parseAndConvert(String[] args) {
        ...
        switch (cqlTypeString.toUpperCase()) {
            case "CONDITION":
                cqlType = CQLTYPES.CONDITION;
                break;
            case "CONDITIONREL":
                cqlType = CQLTYPES.CONDITIONREL;
                break;
            default:
                throw new IllegalArgumentException("Unknown cql type: " + cqlTypeString);
        }
