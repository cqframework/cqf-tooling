...
public class ExecuteMeasureTestArgumentProcessor {
    ...
    public OptionParser build() {
        ...
        OptionSpecBuilder testBundlePathBuilder = parser.acceptsAll(
                asList(TEST_BUNDLE_PATH_OPTIONS), "Test Bundle must conform to the cqf spec for Measure test cases.");
        OptionSpecBuilder contentBundlePathBuilder = parser.acceptsAll(
                asList(CONTENT_BUNDLE_PATH_OPTIONS),
                "Required if running locally. If omitted, existing measure content on the server will be used.");
        ...
        OptionSpecBuilder fhirServerBuilder = parser.acceptsAll(
                asList(FHIR_SERVER_OPTIONS),
                "If omitted, the cql-evaluator will be used locally. NOTE: Not yet implemented");

        OptionSpec<String> testBundlePath =
                testBundlePathBuilder.withRequiredArg().required().describedAs("path to test data bundle");

        OptionSpec<String> fhirServer =
                fhirServerBuilder.withRequiredArg().describedAs("url of fhir server to use for evaluation.");

        OptionSpec<String> contentBundlePath = contentBundlePathBuilder
                .requiredUnless(fhirServer)
                .withRequiredArg()
                .describedAs("path to measure content bundle");

        OptionSpec<FhirVersionEnum> fhirVersion = fhirVersionBuilder
                .withRequiredArg()
                .describedAs("fhir version")
                .withValuesConvertedBy(new FhirVersionEnumConverter())
                .defaultsTo(FhirVersionEnum.R4);
        OptionSpec<Encoding> outputEncoding = outputEncodingBuilder
                .withRequiredArg()
                .describedAs("desired output encoding for resources")
                .withValuesConvertedBy(new EncodingConverter())
                .defaultsTo(Encoding.JSON);
