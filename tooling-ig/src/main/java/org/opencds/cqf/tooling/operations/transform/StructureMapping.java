...
public class StructureMapping implements ExecutableOperation {
    ...
    @OperationParam(
            alias = {"ptpd", "pathtopatientdata"},
            setter = "setPathToPatientData",
            required = true,
            description =
                    "Path to the patient data represented as either a FHIR Bundle resource or as flat files within a directory (required).")
    private String pathToPatientData;

    @OperationParam(
            alias = {"ptsm", "pathtostructuremaps"},
            setter = "setPathToStructureMaps",
            required = true,
            description =
                    "Path to the FHIR StructureMap resource(s) used for the transformation. Can be either a file or directory (required).")
    private String pathToStructureMaps;

    @OperationParam(
            alias = {"purl", "packageurl"},
    ...
    @OperationParam(
            ...
            defaultValue = "json",
            description =
                    "The file format to be used for representing the resulting transformed FHIR resources { json, xml } (default json)")
    ...
    @OperationParam(
            ...
            description =
                    "The directory path to which the transformed FHIR resources should be written (default is to replace existing resources within the IG)")
    ...
    public void execute() {
        ...
        List<IBaseResource> resources = BundleUtil.toListOfResources(
                fhirContext, IOUtils.bundleResourcesInDirectory(pathToPatientData, fhirContext, true));
        for (var resource : resources) {
            StructureMap structureMap = structureMapMap.get(resource.fhirType());
            if (structureMap != null) {
                var target = fhirContext
                        .getResourceDefinition(structureMapUtilities
                                .getTargetType(structureMap)
                                .getType())
                        .newInstance();
                IOUtils.writeResource(
                        transform((Resource) resource, (Resource) target, structureMap),
                        outputPath,
                        IOUtils.Encoding.valueOf(encoding),
                        fhirContext);
            }
        }
    }
        ...
        for (var structureMap : structureMaps) {
            for (var group : structureMap.getGroup()) {
                for (var input : group.getInput()) {
                    if (input.hasType()
                            && input.hasMode()
                            && input.getMode() == StructureMap.StructureMapInputMode.SOURCE) {
    ...
    private List<StructureMap> getStructureMaps() {
        return BundleUtil.toListOfResourcesOfType(
                fhirContext,
                IOUtils.bundleResourcesInDirectory(pathToStructureMaps, fhirContext, true),
                StructureMap.class);
    }
