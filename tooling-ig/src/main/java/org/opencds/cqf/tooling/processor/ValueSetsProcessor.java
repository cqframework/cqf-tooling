...
public class ValueSetsProcessor {
            ...
            if (resource instanceof org.hl7.fhir.dstu3.model.ValueSet) {
                ...
                for (org.hl7.fhir.dstu3.model.Bundle.BundleEntryComponent bundleEntry : bundle.getEntry()) {
                    org.hl7.fhir.dstu3.model.ValueSet valueSet =
                            (org.hl7.fhir.dstu3.model.ValueSet) bundleEntry.getResource();
            ...
            if (resource instanceof org.hl7.fhir.r4.model.ValueSet) {
                ...
                for (org.hl7.fhir.r4.model.Bundle.BundleEntryComponent bundleEntry : bundle.getEntry()) {
                    org.hl7.fhir.r4.model.ValueSet valueSet =
                            (org.hl7.fhir.r4.model.ValueSet) bundleEntry.getResource();
                    valueSetUrls.putIfAbsent((valueSet).getUrl(), valueSet);
                }
            }
        }
    ...
    private static void IntitializeCachedValueSets(FhirContext fhirContext) {
        List<String> allValueSetPaths =
                ResourceDiscovery.getTerminologyPaths(fhirContext).stream().collect(Collectors.toList());
    ...
    public static void bundleValueSets(
        ...
        Map<String, IBaseResource> dependencies = ResourceUtils.getDepValueSetResources(
                cqlContentPath, igPath, fhirContext, includeDependencies, includeVersion);
    ...
    public static void bundleValueSets(
        ...
        Map<String, IBaseResource> dependencies =
                ResourceUtils.getDepValueSetResources(resource, fhirContext, includeDependencies, missingDependencies);
        for (IBaseResource dependency : dependencies.values()) {
            resources.putIfAbsent(
                    resource.fhirType() + '/' + dependency.getIdElement().getIdPart(), resource);
        }
        if (missingDependencies.size() > 0) {
            throw new CqlTranslatorException(
                    missingDependencies.stream().collect(Collectors.toList()),
