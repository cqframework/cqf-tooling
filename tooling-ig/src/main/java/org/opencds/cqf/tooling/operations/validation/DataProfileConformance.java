...
public class DataProfileConformance implements ExecutableOperation {
    ...
    @OperationParam(
            alias = {"purls", "packageurls"},
            setter = "setPackageUrls",
            required = true,
    ...
    @OperationParam(
            ...
            defaultValue = "json",
            description =
                    "The file format to be used for representing the resulting FHIR OperationOutcome { json, xml } (default json)")
    ...
    @OperationParam(
            ...
            description =
                    "The directory path to which the FHIR OperationOutcome should be written (default is to replace existing resources within the IG)")
    ...
    public void execute() {
        ...
        IOUtils.writeResources(
                validatePatientData(patientDataBundle), outputPath, IOUtils.Encoding.valueOf(encoding), fhirContext);
    }
        ...
        for (var patientDataResource : BundleUtil.toListOfResources(fhirContext, patientData)) {
            ...
            if (!result.isSuccessful()) {
                logger.warn(
                        ...
                        resourceType,
                        patientDataResource.getIdElement().getIdPart(),
                        result.getMessages());
    ...
    private void tagResourceWithValidationResult(IBaseResource resource, ValidationResult result) {
        ...
        ExtensionUtil.addExtension(
                fhirContext, resource, Validation.VALIDATION_RESULT_EXTENSION_URL, "Reference", "#" + id);
    ...
    public List<String> getPackageUrlsList() {
        if (packageUrlsList == null && packageUrls != null) {
            packageUrlsList =
                    Arrays.stream(packageUrls.split(",")).map(String::trim).collect(Collectors.toList());
        }
