...
public class PlanDefinitionRefresh extends Refresh {
    ...
    public List<IBaseResource> refresh() {
        ...
        if (getIgInfo().isRefreshPlanDefinitions()) {
            ...
            Class<? extends IBaseResource> clazz = getFhirContext()
            ...
            for (var resource : BundleUtil.toListOfResources(getFhirContext(), bundle)) {
                PlanDefinition planDefinition =
                        (PlanDefinition) ResourceAndTypeConverter.convertToR5Resource(getFhirContext(), resource);
                ...
                LibraryPackage libraryPackage = libraryPackages.stream()
                        .filter(pkg -> libraryUrl.endsWith(
                                pkg.getCqlFileInfo().getIdentifier().getId()))
                        .findFirst()
                        .orElse(null);
                for (CqlProcessor.CqlSourceFileInformation info : cqlProcessor.getAllFileInformation()) {
                    if (libraryUrl.endsWith(info.getIdentifier().getId())) {
                        ...
                        Library moduleDefinitionLibrary = getModuleDefinitionLibrary(planDefinition, dataRecProc, info);
                        ...
                        IBaseResource refreshedPlanDefinition =
                                ResourceAndTypeConverter.convertFromR5Resource(getFhirContext(), planDefinition);
                        refreshedPlanDefinitions.add(refreshedPlanDefinition);
                        this.planDefinitionPackages.add(new PlanDefinitionPackage(
                                planDefinition, refreshedPlanDefinition, getFhirContext(), libraryPackage));
                    }
                }
    ...
    private void resolvePlanDefinitionPackages() {
        ...
        activityDefinitions = BundleUtil.toListOfResources(
                getFhirContext(),
                BundleUtils.getBundleOfResourceTypeFromDirectory(
                        getIgInfo().getActivityDefinitionResourcePath(),
                        getFhirContext(),
                        getFhirContext()
                                .getResourceDefinition("ActivityDefinition")
                                .newInstance()
                                .getClass()));
        questionnaires = BundleUtil.toListOfResources(
                getFhirContext(),
                BundleUtils.getBundleOfResourceTypeFromDirectory(
                        getIgInfo().getActivityDefinitionResourcePath(),
                        getFhirContext(),
    ...
    private void resolveAction(PlanDefinition.PlanDefinitionActionComponent action, PlanDefinitionPackage pkg) {
        ...
        if (definitionRef != null && definitionRef.hasResourceType()) {
            if (definitionRef.getResourceType().equals("ActivityDefinition")) {
                pkg.addActivityDefinition(activityDefinitions.stream()
                        .filter(ad -> ad.getIdElement().getIdPart().equals(definitionRef.getIdPart()))
                        .findFirst()
                        .orElse(null));

            } else if (definitionRef.getResourceType().equals("Questionnaire")) {
                pkg.addQuestionnaire(questionnaires.stream()
                        .filter(q -> q.getIdElement().getIdPart().equals(definitionRef.getIdPart()))
                        .findFirst()
                        .orElse(null));
            } else {
    ...
    private Library getModuleDefinitionLibrary(
            PlanDefinition planDefinition,
            DataRequirementsProcessor dataRecProc,
        ...
        return dataRecProc.gatherDataRequirements(
                ...
                cqlProcessor.getLibraryManager().resolveLibrary(info.getIdentifier(), new ArrayList<>()),
