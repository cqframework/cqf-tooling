...
public class RollDatesDstu3 {
    ...
    public static void rollDatesInResource(IBaseResource resource) {
        ...
        logger.info("resource having date rolled:  "
                + dstu3Resource.getNamedProperty("id").getValues().get(0));
                            ...
                            if (null != propertyTypeCode && !propertyTypeCode.isEmpty()) {
                                if (propertyTypeCode.toLowerCase().contains("datetime")
                                    ...
                                    if (!resourceProperty.getValues().isEmpty()
                                        ...
                                        LocalDate dateToRole =
                                                DataDateRollerUtilsDstu3.getOldDateFromResource(dstu3Resource, field);
                                        if (null == dateToRole) {
                                            continue;
                                        }
                                        LocalDate newLocalDate =
                                                DataDateRollerUtilsDstu3.rollDate(dateToRole, dataDateRollerSettings);
                                        DateTimeType ddType = new DateTimeType();
                                        ZoneId defaultZoneId = ZoneId.systemDefault();
                                        ddType.setValue(Date.from(newLocalDate
                                        ...
                                        Property newProperty = new Property(
                                                oldProperty.getName(),
                                                oldProperty.getTypeCode(),
                                                oldProperty.getDefinition(),
                                                oldProperty.getMinCardinality(),
                                                oldProperty.getMaxCardinality(),
                                                values);
                                        Base newBase = dstu3Resource.makeProperty(
                                                newProperty.getName().hashCode(), newProperty.getName());
                                        if (newProperty.getTypeCode().contains("dateTime")) {
                                            ((DateTimeType) newBase)
                                                    .setValue(Date.from(newLocalDate
                                                            .atStartOfDay(defaultZoneId)
                                                            .toInstant()));
                                ...
                                if (null
                                        ...
                                        && null
                                                != dstu3Resource
                                                        ...
                                                        .getValues()
                                                        .get(0)
                                                        .getNamedProperty("validityPeriod")
                                        && null
                                                != dstu3Resource
                                                        .getNamedProperty(field.getName())
                                                        .getValues()
                                        ...
                                        && dstu3Resource
                                                        ...
                                                        .size()
                                                > 0) {
                                    Period newPeriod = rollPeriodDates(
                                            dstu3Resource
                                                    .getNamedProperty(field.getName())
                                                    .getValues()
                                                    .get(0)
                                                    .getNamedProperty("validityPeriod"),
                                            dataDateRollerSettings);
    ...
    private static void handlePeriod(
            Resource resource, DataDateRollerSettingsDstu3 dataDateRollerSettings, String fieldName) {
