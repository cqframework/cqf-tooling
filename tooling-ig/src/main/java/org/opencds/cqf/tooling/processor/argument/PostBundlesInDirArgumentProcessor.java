...
public class PostBundlesInDirArgumentProcessor {
    ...
    public OptionParser build() {
        ...
        OptionSpecBuilder dirPathBuilder =
                parser.acceptsAll(asList(DIRECTORY_PATH_OPTIONS), "Limited to a single version of FHIR.");
        OptionSpecBuilder fhirVersionBuilder =
                parser.acceptsAll(asList(FHIR_VERSION_OPTIONS), "As of now FHIR DSTU3 and R4 are supported");
        OptionSpecBuilder encodingBuilder = parser.acceptsAll(
                asList(ENCODING_OPTIONS), "If omitted, output will be generated using JSON encoding.");
        OptionSpecBuilder fhirUriBuilder = parser.acceptsAll(asList(FHIR_URI_OPTIONS), "FHIR server.");

        OptionSpec<String> dirPath =
                dirPathBuilder.withRequiredArg().describedAs("path to directory containing bundle resources");
