...
public class DataRequirementsProvider {
    ...
    //        Map<VersionedIdentifier, Pair<Library, Library>> libraryMap = this.createLibraryMap(measure,
    // libraryResourceProvider);
    //        return this.createCqfMeasure(measure, libraryMap);
    //    }
    //
    //    private Map<VersionedIdentifier, Pair<Library, Library>>  createLibraryMap(Measure measure,
    ...
    //    }
    //
    //    private CqfMeasure createCqfMeasure(Measure measure, Map<VersionedIdentifier, Pair<Library, Library>>
    // libraryMap)
    //    {
    //        //Ensure All Data Requirements for all referenced libraries
    //        Library moduleDefinition = this.getDataRequirements(measure,
    //            libraryMap.values().stream().map(x -> x.getRight()).filter(x -> x !=
    ...
    //            Boolean isPrimaryLibrary = libraryResource != null &&
    // libraryResource.getId().equals(primaryLibraryId);
    ...
    //
    //                    CqfMeasure.TerminologyRef term = new
    // CqfMeasure.VersionedTerminologyRef(TerminologyRefType.CODESYSTEM, name, codeId, version);
    ...
    //                    }
    //
    //                    CqfMeasure.TerminologyRef term = new CqfMeasure.CodeTerminologyRef(name, codeId,
    // codeSystemName, codeSystemId, displayName);
    //                    Boolean exists = false;
    ...
    //
    //                    CqfMeasure.TerminologyRef term = new
    // CqfMeasure.VersionedTerminologyRef(TerminologyRefType.VALUESET, name, valueSetId);
    //                    Boolean exists = false;
    ...
    //                            if (filter.hasValueSetStringType() &&
    // filter.getValueSetStringType().getValueAsString().equalsIgnoreCase(valueSet.getId())) {
    //                                StringType dataElement = new StringType();
    //                                dataElement.setValueAsString("\"" + type + ": " + name + "\" using \"" + name + "
    // (" + valueSetId  + ")");
    //                                exists = false;
    //                                for (StringType string : dataCriteria) {
    //                                    if
    // (string.getValueAsString().equalsIgnoreCase(dataElement.getValueAsString())) {
    //                                        exists = true;
    //                                    }
    //                                }
    //                                if (!exists) {
    ...
    //            }
    //
    //            // Don't try to parse statements for libraries that don't have resources (such as an embedded
    // FHIRHelpers)
    ...
    //                            signature = cqlLines[i].substring(cqlLines[i].indexOf("("), cqlLines[i].indexOf(")") +
    // 1);
    //                        }
    ...
    //            }
    //        };
    //        Comparator<MeasureGroupPopulationComponent> populationComparator = new
    // Comparator<MeasureGroupPopulationComponent>() {
    //            @Override
    ...
    //        cqfMeasure.setLibraries(libraryMap.values().stream().map(x -> x.getRight()).filter(x -> x !=
    // null).collect(Collectors.toList()));
    ...
    //                    }
    //
    //                    criteriaMap.get(criteria).add(Triple.of(i, mgpc.getCode().getCodingFirstRep().getCode(),
    // mgpc.getDescription()));
    //                }
    //            }
    //        }
    ...
    //            if (cqfMeasure.getGroup().size() == 1 || entry.getValue().stream().map(x ->
    // x.getLeft()).distinct().count() > 1) {
    ...
    // !cqfMeasure.getSharedPopulationCriteria().containsKey(mgpc.getCriteria())) {
    ...
    //        // options.set(AnchorLinkExtension.ANCHORLINKS_TEXT_PREFIX, "<span class=\"octicon
    // octicon-link\"></span>");
    //
    ...
    //        measure.setClinicalRecommendationStatement(markdownToHtml(parser, renderer,
    ...
    //        Map<VersionedIdentifier, Pair<Library, Library>> libraryMap = this.createLibraryMap(measure,
    // libraryResourceProvider);
    //        return this.getDataRequirements(measure, libraryMap.values().stream().map(x -> x.getRight()).filter(x -> x
    // != null).collect(Collectors.toList()));
    //    }
    //
    ...
    public void ensureRelatedArtifacts(
            Library library, CqlTranslator translator, LibraryResourceProvider libraryResourceProvider) {
            ...
            for (org.hl7.elm.r1.IncludeDef def : elm.getIncludes().getDef()) {
                library.addRelatedArtifact(new RelatedArtifact()
                        ...
                        .setResource(new Reference()
                                .setReference(libraryResourceProvider
        ...
        for (org.hl7.elm.r1.Retrieve retrieve : translator.toRetrieves()) {
            ...
            if (retrieve.getCodeProperty() != null) {
                ...
                if (retrieve.getCodes() instanceof ValueSetRef) {
                    Type valueSetName =
                            new StringType(getValueSetId(((ValueSetRef) retrieve.getCodes()).getName(), translator));
