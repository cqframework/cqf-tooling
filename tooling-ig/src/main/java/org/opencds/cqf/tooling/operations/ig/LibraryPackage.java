...
public class LibraryPackage {
    ...
    public LibraryPackage(
            IBaseResource library, FhirContext fhirContext, CqlProcessor.CqlSourceFileInformation cqlFileInfo) {
    ...
    public void addDependsOnLibrary(IBaseResource library) {
        if (library != null
                && this.dependsOnLibraries.stream()
                        .noneMatch(dep -> ResourceUtils.compareResourceIdUrlAndVersion(library, dep, fhirContext))) {
    ...
    public void addDependsOnValueSet(IBaseResource valueSet) {
        if (valueSet != null
                && this.dependsOnValueSets.stream()
                        .noneMatch(dep -> ResourceUtils.compareResourceIdUrlAndVersion(valueSet, dep, fhirContext))) {
    ...
    public void addDependsOnCodeSystem(IBaseResource codeSystem) {
        // TODO: CodeSystems are extensible... Possible for multiple with the same ID - currently just including the
        // first
        if (codeSystem != null
                && this.dependsOnCodeSystems.stream()
                        .noneMatch(dep ->
                                ResourceUtils.compareResourcePrimitiveElements(codeSystem, dep, fhirContext, "id"))) {
