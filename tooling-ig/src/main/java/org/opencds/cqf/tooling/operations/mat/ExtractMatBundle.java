...
public class ExtractMatBundle implements ExecutableOperation {
    ...
    @OperationParam(
            ...
            defaultValue = "json",
            description =
                    "The file format to be used for representing the resulting extracted FHIR resources { json, xml } (default json)")
    ...
    @OperationParam(
            alias = {"sn", "suppressnarrative"},
            setter = "setSuppressNarrative",
            defaultValue = "true",
    ...
    @OperationParam(
            ...
            description =
                    "The directory path to which the generated Bundle file should be written (default parent directory of -ptb)")
    ...
    public MatPackage getMatPackage(IBaseBundle bundle) {
        ...
        for (IBaseResource resource : resources) {
            if (resource.fhirType().equalsIgnoreCase("measure") && Boolean.TRUE.equals(suppressNarrative)) {
                ResourceUtil.removeNarrative(fhirContext, resource);
            }
            if (resource.fhirType().equalsIgnoreCase("library")) {
                matPackage.addLibraryPackage(new MatPackage.LibraryPackage()
                        .setLibrary(resource)
                        .setCql(new String(Base64.decodeBase64(extractCql(resource, terser)))));
