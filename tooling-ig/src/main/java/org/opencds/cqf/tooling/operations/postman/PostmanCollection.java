package org.opencds.cqf.tooling.operations.postman;
...
@Operation(name = "PostmanCollection")
public class PostmanCollection implements ExecutableOperation {
    ...
    @OperationParam(
            alias = {"ptbd", "pathtobundledir"},
            setter = "setPathToBundlesDir",
            required = true,
            description = "Path to the directory containing bundle files (required)")
    ...
    @OperationParam(
            ...
            setter = "setVersion",
            required = true,
            description = "FHIR version { dstu3, r4 } (required)")
    private String version;

    @OperationParam(
            alias = {"op", "outputpath"},
            setter = "setOutputPath",
            description = "The directory path to which the Postman collection should be written (optional)")
    ...
    @OperationParam(
            ...
            setter = "setProtocol",
            defaultValue = "http",
            description = "The URL protocol { http, https } (default http)")
    private String protocol;

    @OperationParam(
            alias = {"host"},
            setter = "setHost",
            description = "The URL host (e.g. 'localhost:8080') (optional, defaults to '{server-base}')")
    private String host;

    @OperationParam(
            alias = {"path"},
            setter = "setPath",
            description = "The URL path (e.g. 'fhir') (optional, defaults to '{path}')")
    private String path;

    @OperationParam(
            alias = {"name"},
            setter = "setCollectionName",
            description = "Name for the Postman collection (optional, defaults to timestamped name)")
    private String collectionName;

    private FhirContext context;

    private List<String> hostTokens;
    private StringBuffer hostNames;
    private List<String> pathTokens;
    private StringBuffer pathNames;
    ...
    public void execute() {
        ...
        try {
            ...
            PostmanCollectionModel postmanCollection = createPostmanCollection();
    ...
    private List<String> generateUrlHostTokens() {
        try {
            if (hostTokens == null) {
                hostTokens = new ArrayList<>();
                hostNames = new StringBuffer();
                if (!StringUtils.isEmpty(host)) {
                    hostTokens.addAll(Arrays.asList(host.split("\\.")));
                }
    ...
    private List<String> generateUrlPathTokens() {
        try {
            if (pathTokens == null) {
                ...
                if (!StringUtils.isEmpty(path)) {
                    pathTokens.addAll(Arrays.asList(path.split("/")));
                }
    ...
    private PostmanCollectionModel createPostmanCollection() {
        PostmanCollectionModel postmanCollection = new PostmanCollectionModel();
    ...
    private BaseItem populateVersionItem(PostmanCollectionModel postmanCollection, String version) {
    ...
    private void generateSubfolderItem(BaseItem itemSubFolder, String id, String content) {
        itemSubFolder.setName(id.split("-")[0]);
    ...
    private List<Map<String, String>> generateRequestHeaderMap() {
    ...
    private void writePostmanCollection(PostmanCollectionModel postmanCollection) {
        ...
        try {
            ...
            BufferedWriter writer =
                    new BufferedWriter(new FileWriter(String.format("%s/%s.json", outputPath, createDefaultName())));
    ...
    public void setPathToBundlesDir(String pathToBundlesDir) {
        this.pathToBundlesDir = pathToBundlesDir;
    }
    ...
    // Inner classes for Postman data structures
