...
public class FileFhirDal implements FhirDal {
    ...
    public void create(IBaseResource resource) {
        if (resourceTypeDefined(resource)) {
            ...
            String typePath = IOUtils.concatFilePath(
                    this.resourceDir, resource.getIdElement().getResourceType());
    ...
    private void writeResource(String path, IBaseResource resource) {
        ...
        try (FileOutputStream writer = new FileOutputStream(path)) {
            writer.write(
                    parser.setPrettyPrint(true).encodeResourceToString(resource).getBytes());
            writer.flush();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private String getPath(IIdType id) {
        return IOUtils.concatFilePath(
                this.resourceDir, id.getResourceType(), id.getIdPart() + "." + this.encoding.toString());
    }
    ...
    private boolean resourceTypeDefined(IBaseResource resource) {
        if (resource.getIdElement().hasResourceType()) {
            ...
            logger.warn(
                    "ResourceType not defined for: {}", resource.getIdElement().getIdPart());
            return false;
        }
    }
