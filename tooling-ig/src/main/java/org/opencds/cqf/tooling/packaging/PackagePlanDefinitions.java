...
public class PackagePlanDefinitions {
    ...
    public PackagePlanDefinitions(
        ...
        var pdResourcePaths = ResourceDiscovery.getPlanDefinitionPaths(fhirContext);
        if (fhirContext.getVersion().getVersion() == FhirVersionEnum.R4) {
            pdResourcePaths.forEach(path -> new PackagePlanDefinition(
                            igRoot,
                            fhirContext,
                            path,
                            includeDependencies,
            ...
            throw new UnsupportedOperationException(
                    "Package operation for PlanDefinition resources is not supported for FHIR version: "
