...
public class LibraryRefresh extends Refresh {
    ...
    public LibraryRefresh(IGInfo igInfo) {
        ...
        this.cqlProcessor = new CqlProcessor(
                packageList,
    ...
    public List<IBaseResource> refresh(RefreshIGParameters params) {
        ...
        if (getIgInfo().isRefreshLibraries()) {
            ...
            for (var library :
                    getResourcesOfTypeFromDirectory("Library", getIgInfo().getLibraryResourcePath())) {
    ...
    private void resolveLibraryPackages() {
        ...
        this.libraryPackages.forEach(libraryPackage -> {
            libraryPackage.setDependsOnValueSets(sourceIGValueSets);
            libraryPackage.setDependsOnCodeSystems(sourceIGCodeSystems);
            libraryPackage.getCqlFileInfo().getRelatedArtifacts().forEach(relatedArtifact -> {
                if (relatedArtifact.hasResource() && UrlUtil.isValid(relatedArtifact.getResource())) {
                    ...
                    if (relatedArtifact.getResource().contains("/Library/")) {
                        ...
                        if (identifier.getSystem().equals(getIgInfo().getCanonical())) {
                            ...
                            libraryPackage.addDependsOnLibrary(
                                    getLibraryPackage(identifier).getLibrary());
                        } else {
                    ...
                    //                                  identifier =
                    // CanonicalUtils.toVersionedIdentifierAnyResource(relatedArtifact.getResource());
                    //                                  if (!identifier.getSystem().equals(getIgInfo().getCanonical()))
                    // {
                    //
    ...
    private LibraryPackage getLibraryPackage(VersionedIdentifier identifier) {
        return this.libraryPackages.stream()
                .filter(pkg -> pkg.getCqlFileInfo().getIdentifier().equals(identifier))
                .findFirst()
                .orElse(null);
    }
    ...
    private LibraryPackage getLibraryPackage(String url) {
        return this.libraryPackages.stream()
                .filter(pkg -> url.endsWith(pkg.getLibrary().getIdElement().getIdPart()))
                .findFirst()
                .orElse(null);
    }
    ...
    private IBaseResource getResourceFromNpmPackage(
            VersionedIdentifier identifier, String resourceType, Map<String, List<IBaseResource>> resourceCache) {
        String url;
            ...
            if (npmPackage != null && npmPackage.getFolders().containsKey("package")) {
                ...
                try {
                    if (npmPackage.getFolders().get("package").getTypes().containsKey(resourceType)) {
                        resourceCache.put(
                                url,
                                npmPackage.getFolders().get("package").getTypes().get(resourceType).stream()
                                        .map(fileName -> IOUtils.readJsonResourceIgnoreElements(
        ...
        if (resourceCache.containsKey(url)) {
            return resourceCache.get(url).stream()
                    .filter(resource -> {
    ...
    private List<NpmPackage> clearCollisions(List<NpmPackage> collisionPossibleList, IGInfo igInfo) {
        ...
        for (NpmPackage pkg : collisionPossibleList) {
            for (NpmPackage innerPkg : collisionPossibleList) {
                if (pkg.id().equals(innerPkg.id())
                        && pkg.canonical().equals(innerPkg.canonical())
                        && !pkg.version().equals(innerPkg.version())) {
                    // if igInfo has the version we want, use it, otherwise use latest
                    var explicit = igInfo.getIgResource().getDependsOn().stream()
                            .filter(x -> x.getPackageId().equals(pkg.id()))
                            .findFirst();
    ...
    private void refreshContent(IBaseResource library, CqlProcessor.CqlSourceFileInformation info) {
        ...
        Attachment elmXml =
                new Attachment().setContentType("application/elm+xml").setData(info.getElm());
        Attachment elmJson =
                new Attachment().setContentType("application/elm+json").setData(info.getJsonElm());
