...
public class BundleTransaction implements ExecutableOperation {
    ...
    @OperationParam(
            alias = {"ptb", "pathtobundles"},
            setter = "setPathToBundles",
            required = true,
    ...
    public void execute() {
        ...
        List<IBaseBundle> bundles = IOUtils.readResources(IOUtils.getFilePaths(pathToBundles, true), context).stream()
                .filter(IBaseBundle.class::isInstance)
                .map(IBaseBundle.class::cast)
                .collect(Collectors.toList());
        bundleTransaction(bundles, context, fhirServer);
    }

    public static List<IBaseBundle> bundleTransaction(
            @Nonnull List<IBaseBundle> bundles, @Nonnull FhirContext fhirContext, @Nonnull String fhirServerUri) {
