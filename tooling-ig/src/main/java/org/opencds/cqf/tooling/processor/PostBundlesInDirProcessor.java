...
public class PostBundlesInDirProcessor {
    ...
    public static void PostBundlesInDir(PostBundlesInDirParameters params) {
        ...
        List<Map.Entry<String, IBaseResource>> resources =
                BundleUtils.getBundlesInDir(params.directoryPath, fhirContext);
    ...
    private static void postBundleToFhirUri(
            String fhirUri, Encoding encoding, FhirContext fhirContext, IBaseResource bundle) {
        if (fhirUri != null && !fhirUri.equals("")) {
            try {
                HttpClientUtils.post(fhirUri, bundle, encoding, fhirContext, null);
                logger.info(
                        "Resource successfully posted to FHIR server ({}): {}",
                        fhirUri,
                        bundle.getIdElement().getIdPart());
            } catch (Exception e) {
                logger.error(
                        "Error occurred for element {}: {}",
                        bundle.getIdElement().getIdPart(),
                        e.getMessage());
            }
        }
