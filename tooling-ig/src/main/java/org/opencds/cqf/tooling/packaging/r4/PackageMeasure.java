...
public class PackageMeasure extends Package<Measure> {
    ...
    public TestPackage<Group, Bundle> resolveTests(Measure mainArtifact) {
        ...
        for (var candidateDirectory : candidateDirectories) {
            if (candidateDirectory.endsWith(mainArtifact.getIdElement().getIdPart())) {
                ...
                var groups =
                        IOUtils.getResourcesOfTypeInDirectory(candidateDirectory, getFhirContext(), Group.class, true);
                if (!groups.isEmpty()) {
                ...
                for (var testDirectory : IOUtils.getDirectoryPaths(candidateDirectory, true)) {
                    var testBundle =
                            IOUtils.bundleResourcesInDirectoryAsTransaction(testDirectory, getFhirContext(), true);
    ...
    */
    ...
    public void output() {
        ...
        var measureOutputPath = IOUtils.concatFilePath(getBundleOutputPath(), "measure", measureId);
        IOUtils.initializeDirectory(measureOutputPath);

        var measureFilesOutputPath = IOUtils.concatFilePath(measureOutputPath, measureId + "-files");
        ...
        if (getPrimaryLibrary() != null) {
            IOUtils.writeResource(
                    getPrimaryLibrary(), measureFilesOutputPath, IOUtils.Encoding.JSON, getFhirContext());
        }
        ...
        if (getPrimaryLibrary() != null) {
            var cqlFileOutputPath = IOUtils.concatFilePath(
                    measureFilesOutputPath, ((Library) getPrimaryLibrary()).getIdPart() + ".cql");
            IOUtils.writeCqlToFile(
                    ResourceUtils.getCqlFromR4Library((Library) getPrimaryLibrary()), cqlFileOutputPath);
        }
        ...
        IOUtils.writeBundle(
                packageBundle, measureOutputPath, IOUtils.Encoding.JSON, getFhirContext(), measureId + "-bundle");
