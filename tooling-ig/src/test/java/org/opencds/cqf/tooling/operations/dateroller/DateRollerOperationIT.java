...
public class DateRollerOperationIT {
    ...
    void testDateRollerDateTimeElement() {
        ...
        boolean result =
                dateRoller.getAllDateElements(fhirContext, observation, dateRoller.getDateClasses(fhirContext));
        Assert.assertTrue(result);
        Assert.assertTrue(
                DateUtils.isSameDay(observation.getEffectiveDateTimeType().getValue(), today));
        Assert.assertTrue(DateUtils.isSameDay(
                ((DateTimeType) observation
                                .getExtensionByUrl(RollTestDates.DATEROLLER_EXT_URL)
                                .getExtensionByUrl("dateLastUpdated")
                                .getValue())
                        .getValue(),
                today));
    }
    ...
    void testDateRollerPeriodElementUTCTimezone() {
        ...
        Assert.assertTrue(result);
        Assert.assertTrue(
                DateUtils.isSameDay(observation.getEffectivePeriod().getStart(), DateUtils.addDays(today, -1)));
        Assert.assertTrue(DateUtils.isSameDay(observation.getEffectivePeriod().getEnd(), today));
        // Check that TimeZone is preserved
        Assert.assertEquals(
                observation.getEffectivePeriod().getStartElement().getTimeZone().getRawOffset(), 0);
    }
    ...
    void testDateRollerBackboneElement() {
        ...
        Assert.assertTrue(result);
        Assert.assertTrue(DateUtils.isSameDay(
                observation.getComponentFirstRep().getValueDateTimeType().getValue(), DateUtils.addDays(today, -10)));
    }
