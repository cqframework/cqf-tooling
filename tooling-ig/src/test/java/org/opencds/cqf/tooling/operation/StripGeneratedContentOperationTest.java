...
public class StripGeneratedContentOperationTest {
    ...
    public void test_strip_generated_content() throws URISyntaxException, FileNotFoundException {
        ...
        var inputFilePath = Path.of(StripGeneratedContentOperationTest.class
                .getResource(dataInputPath)
                .toURI());
        var outputPath = Path.of("target", "test-output", "strip-generated-content");
        String version = "r4";

        Library libraryBeforeStrip = (Library) FhirContext.forR4Cached()
                .newJsonParser()
                .parseResource(new FileReader(inputFilePath + "/LibraryBreastCancerScreeningFHIR.json"));
        ...
        File jsonFile =
                outputPath.resolve("LibraryBreastCancerScreeningFHIR.json").toFile();

        var libraryAfterStrip =
                (Library) FhirContext.forR4Cached().newJsonParser().parseResource(new FileReader(jsonFile));
    ...
    void exportsCql() throws URISyntaxException, FileNotFoundException {
        ...
        File cqlFile = outputPath
                .resolve("cql")
                .resolve("BreastCancerScreeningFHIR.cql")
                .toFile();
